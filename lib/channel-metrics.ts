...
export async function refreshYoutubeMetrics(force = false) {
  ...
  const stored = await db.prepare('SELECT canonical_key, subscriber_count, updated_at FROM channel_metrics').all<ChannelMetric>();
  const storedByKey = new Map((stored.results ?? []).map((item) => [item.canonical_key, item]));
  const cutoff = Date.now() - 86_400_000;
  const stale = [...identities]
    .filter((identity) => force || !storedByKey.get(identity)?.updated_at || Date.parse(storedByKey.get(identity)!.updated_at) < cutoff)
    .slice(0, force ? undefined : 5);
  if (!stale.length) return;

  const now = new Date().toISOString();
  const save = async (canonicalKey: string, channel?: YoutubeChannel) => {
    const freshCount = Number(channel?.statistics?.subscriberCount ?? '');
    const count = channel && !channel.statistics?.hiddenSubscriberCount && Number.isFinite(freshCount)
      ? freshCount
      : storedByKey.get(canonicalKey)?.subscriber_count ?? 0;
  ...
  const channelIds = stale.filter((item) => item.startsWith('youtube:channel/'));
  for (let index = 0; index < channelIds.length; index += 50) {
    ...
    try {
      ...
      await Promise.all(batch.map((identity) => save(identity)));
    }
  }
  const handles = stale.filter((item) => item.startsWith('youtube:') && !item.startsWith('youtube:channel/'));
