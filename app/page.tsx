    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 flex justify-center px-5 pt-7">
        {item.image ? (
          ...
          <div
            aria-label={`Лагатып канала ${item.title}`}
...
function CarouselRow({ items }: { items: Media[] }) {
  ...
  const itemsKey = items
    .map((item) => `${item.platform}:${item.url ?? item.title}`)
    .join('|');
  ...
  return (
    <div className="relative">
      ...
      >
        {items.map((item) => (
          <MediaCard key={`${item.platform}-${item.title}`} item={item} />
        ))}
      </div>
      {showControls && (
        <>
        ...
        {sent ? (
          <div className="py-8 text-center">
            ...
            <DialogDescription className="mt-3">
              Прапанова захавана для праверкі рэдакцыяй КОШа.
            </DialogDescription>
          </div>
        ) : (
          <>
            <DialogHeader>
              ...
              <DialogDescription>
                Дашлі спасылку на добры беларускамоўны канал, ролік або падкаст.
              </DialogDescription>
            </DialogHeader>
            <form className="mt-2 space-y-4" onSubmit={submitSuggestion}>
              ...
              >
                ...
                {sending ? 'Адпраўляем…' : 'Адправіць прапанову'}
              </Button>
            </form>
          </>
        )}
    ...
    <main className="site-shell min-h-screen overflow-x-hidden text-foreground">
      <header className="fixed inset-x-0 top-0 z-40 border-b border-white/10 bg-[#111821]/82 backdrop-blur-xl">
        <div className="mx-auto flex h-18 max-w-[1500px] items-center gap-7 px-5 lg:px-10">
          <Sheet>
            ...
            <SheetContent side="left" className="border-white/10 bg-[#101115]">
              <SheetHeader>
                ...
                <SheetDescription>
                  Беларускі кантэнт у адным кошы
                </SheetDescription>
              </SheetHeader>
              <nav className="grid gap-1 px-4 text-lg">
                <a className="rounded-xl bg-white/6 px-4 py-3" href="#">
                  Галоўная
                </a>
                <a className="rounded-xl px-4 py-3" href="#catalog">
                  Катэгорыі
                </a>
                <a className="rounded-xl px-4 py-3" href="#new">
                  Новае
                </a>
                <a className="rounded-xl px-4 py-3" href="#about">
                  Пра КОШ
                </a>
              </nav>
            </SheetContent>
          ...
          <nav
            ...
            aria-label="Асноўная навігацыя"
          >
            <a className="font-medium text-white" href="#">
              Галоўная
            </a>
            <a className="transition hover:text-white" href="#catalog">
              Катэгорыі
            </a>
            <a className="transition hover:text-white" href="#new">
              Новае
            </a>
          ...
          <div className="ml-auto hidden w-full max-w-xs md:block">
            <div className="relative">
              ...
              <Input
                ...
                aria-label="Пошук па КОШы"
                placeholder="Шукаць кантэнт"
      ...
      <section className="relative isolate min-h-[690px] pt-24">
        <img
          ...
          alt="Ранішняе святло над краявідам"
        />
        ...
        <div className="mx-auto flex min-h-[620px] max-w-[1500px] items-end px-5 pb-16 lg:px-10">
          <div className="max-w-3xl">
            <h1 className="text-balance text-5xl font-black leading-[0.94] tracking-[-0.055em] text-white sm:text-7xl">
              Беларускае —<br />
              бліжэй, чым здаецца
            </h1>
        ...
        {results.length ? (
          <>
            ...
            <div className="mt-3 flex justify-center">
              ...
              >
                Адкрыць каталог <ArrowRight className="size-4" />
              </Button>
            </div>
          </>
        ) : (
          <div className="rounded-3xl border border-dashed border-white/12 bg-white/3 px-6 py-14 text-center">
            ...
            <h3 className="font-bold">Нічога не знайшлося</h3>
            <p className="mt-2 text-sm text-white/50">
              Паспрабуй іншыя словы або абяры «Усё».
            </p>
            <Button
              variant="link"
      ...
      <section id="new" className="mx-auto max-w-[1500px] px-5 py-12 lg:px-10">
        <div className="mb-6 flex items-end justify-between">
          ...
          <button className="hidden items-center gap-1 text-sm font-semibold text-white/55 sm:flex">
            Глядзець усе <ChevronRight className="size-4" />
          </button>
        </div>
        <CarouselRow
          items={
      ...
      >
        <div className="mx-auto grid max-w-5xl gap-8 px-6 py-16 text-center sm:px-12">
          <p className="mx-auto text-xs font-bold uppercase tracking-[0.2em] text-secondary">
            Супольны праект
          </p>
          <h2 className="text-balance text-3xl font-black tracking-tight sm:text-5xl">
            Добры кантэнт ствараюць людзі
          </h2>
          <p className="mx-auto max-w-2xl text-white/62">
            КОШ расце з вашых парад. Калі ведаеш аўтара, канал або падкаст
            па-беларуску — падзяліся спасылкай.
          </p>
          <div>
            <SubmitDialog />
          </div>
      ...
      <footer className="border-t border-white/8">
        <div className="mx-auto flex max-w-[1500px] flex-col gap-4 px-5 py-9 text-sm text-white/45 sm:flex-row sm:items-center sm:justify-between lg:px-10">
          ...
          <p>Зроблена для тых, хто шукае сваё.</p>
        </div>
      </footer>
    </main>
  );
