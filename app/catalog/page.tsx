...
export default function CatalogPage() {
  ...
  const results = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sort === 'newest') return (b.addedAt ?? '').localeCompare(a.addedAt ?? '') || a.title.localeCompare(b.title, 'be');
  ...
  const hasFilters = Boolean(query || selectedPlatforms.length || category !== 'Усе катэгорыі');
    ...
    <main className="site-shell min-h-screen text-foreground">
      <header className="sticky top-0 z-40 border-b border-white/10 bg-[#111821]/92 backdrop-blur-xl">
        <div className="mx-auto flex h-18 max-w-[1500px] items-center gap-5 px-5 lg:px-10">
          ...
          <span className="hidden text-sm text-white/35 sm:block">/ Каталог</span>
          <Button render={<a href="/" />} variant="ghost" className="ml-auto rounded-full text-white/65"><ArrowLeft className="size-4" /> На галоўную</Button>
        </div>
      </header>
      ...
      <div className="mx-auto max-w-[1500px] px-5 py-10 lg:px-10">
        ...
        <div className="grid gap-8 lg:grid-cols-[250px_minmax(0,1fr)]">
          <aside className="h-fit space-y-7 lg:sticky lg:top-28">
            <div className="flex items-center justify-between"><h2 className="flex items-center gap-2 font-bold"><SlidersHorizontal className="size-4 text-secondary" /> Фільтры</h2>{hasFilters && <Button variant="ghost" size="sm" onClick={resetFilters} className="text-white/45"><X className="size-3.5" /> Скінуць</Button>}</div>

            <div>
              <label htmlFor="catalog-search" className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/45">Пошук</label>
              <div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-white/35" /><Input id="catalog-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Назва або тэма" className="h-11 border-white/10 bg-white/5 pl-9" /></div>
            </div>
            ...
            <div>
              <label htmlFor="category" className="mb-2 block text-xs font-bold uppercase tracking-wider text-white/45">Катэгорыя</label>
              <select id="category" value={category} onChange={(event) => setCategory(event.target.value)} className="h-11 w-full rounded-xl border border-white/10 bg-[#151b24] px-3 text-sm text-white outline-none focus:border-secondary/60">{categories.map((name) => <option key={name}>{name}</option>)}</select>
            </div>
          </aside>

          <section>
            <div className="mb-6 flex flex-col gap-3 border-b border-white/8 pb-5 sm:flex-row sm:items-center sm:justify-between">
              <p className="text-sm text-white/50"><strong className="text-white">{results.length}</strong> {results.length === 1 ? 'вынік' : 'вынікаў'}</p>
              <label className="flex items-center gap-3 text-sm text-white/50">Сартаваць<select value={sort} onChange={(event) => setSort(event.target.value as SortKey)} className="h-10 rounded-xl border border-white/10 bg-[#151b24] px-3 text-sm text-white outline-none focus:border-secondary/60"><option value="popular">Па папулярнасці</option><option value="az">Назва: А–Я</option><option value="za">Назва: Я–А</option><option value="newest">Спачатку новыя</option><option value="platform">Па платформе</option></select></label>
            </div>

            {results.length ? <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">{results.map((item) => <MediaCard key={`${item.platform}-${item.title}`} item={item} fluid />)}</div> : <div className="rounded-3xl border border-dashed border-white/12 px-6 py-20 text-center"><Search className="mx-auto mb-4 size-7 text-white/30" /><h2 className="font-bold">Нічога не знайшлося</h2><p className="mt-2 text-sm text-white/45">Змяні фільтры або скінь іх.</p><Button variant="link" onClick={resetFilters} className="mt-2 text-secondary">Скінуць фільтры</Button></div>}
          </section>
        </div>
      </div>
    </main>
