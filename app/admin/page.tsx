  ...
  return (
    <main className="site-shell min-h-screen text-foreground">
      ...
      <div className="mx-auto max-w-6xl px-5 py-10">
        <div className="mb-8"><p className="mb-2 flex items-center gap-2 text-xs font-bold uppercase tracking-[0.18em] text-secondary"><ShieldCheck className="size-4" /> Закрыты раздзел</p><h1 className="text-4xl font-black tracking-tight text-white">Прапанаваныя каналы</h1><p className="mt-3 text-white/50">Правярай спасылкі і прымай рашэнне перад даданнем у каталог.</p></div>
        {loading ? <p className="text-white/45">Загружаем заяўкі…</p> : forbidden ? <div className="rounded-3xl border border-red-300/15 bg-red-300/5 p-8"><h2 className="font-bold text-red-200">Няма доступу</h2><p className="mt-2 text-sm text-white/50">Гэты раздзел даступны толькі ўладальніку КОШа.</p></div> : !items.length ? <div className="rounded-3xl border border-dashed border-white/12 py-20 text-center"><Clock3 className="mx-auto mb-4 size-8 text-white/25" /><h2 className="font-bold">Новых прапаноў пакуль няма</h2></div> : <div className="space-y-4">{items.map((item) => <article key={item.id} className="rounded-2xl border border-white/10 bg-white/4 p-5"><div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between"><div className="min-w-0"><span className={`mb-3 inline-flex rounded-full px-3 py-1 text-xs font-bold ${item.status === 'pending' ? 'bg-amber-300/15 text-amber-200' : item.status === 'approved' ? 'bg-emerald-300/15 text-emerald-200' : 'bg-red-300/15 text-red-200'}`}>{statusLabels[item.status]}</span><a href={item.url} target="_blank" rel="noreferrer" className="flex items-center gap-2 break-all font-bold text-white hover:text-secondary">{item.url}<ExternalLink className="size-4 shrink-0" /></a><p className="mt-3 text-sm leading-relaxed text-white/60">{item.reason}</p><p className="mt-3 text-xs text-white/30">{new Date(item.created_at).toLocaleString('be-BY')}{item.submitter_email ? ` · ${item.submitter_email}` : ''}</p></div><div className="flex shrink-0 gap-2"><Button onClick={() => updateStatus(item.id, 'approved')} className="rounded-full bg-emerald-600 text-white hover:bg-emerald-500"><Check className="size-4" /> Прыняць</Button><Button onClick={() => updateStatus(item.id, 'rejected')} variant="outline" className="rounded-full border-red-300/20 text-red-200 hover:bg-red-300/10"><X className="size-4" /> Адхіліць</Button></div></div></article>)}</div>}
      </div>
    </main>
  );
}
