...
function parseSubscriberCount(html: string) {
  const candidates = [
    ...Array.from(html.matchAll(/"subscriberCountText"\s*:\s*\{[^}]*"simpleText"\s*:\s*"([^"]+)"/gi), (match) => match[1]),
    ...Array.from(html.matchAll(/([\d.,]+)\s*([KMB])?\s+subscribers/gi), (match) => `${match[1]}${match[2] ?? ''}`),
  ];
  if (!candidates.length) return null;
  const values = candidates.map((candidate) => {
    const match = candidate.replaceAll(',', '').replace(/\s/g, '').match(/([\d.]+)([KMB])?/i);
    if (!match) return 0;
...
function parseStoredSubscribers(value?: string) {
  ...
  return value.includes('млн') ? number * 1_000_000 : value.includes('тыс') ? number * 1_000 : number;
}
...
async function refreshStats(db: Awaited<ReturnType<typeof ensureSubmissionsTable>>, previous?: StatsPayload) {
  ...
  let subscriberCount = Math.max(parseStoredSubscribers(previous?.topSubscribers), topChannel.fallback);
  try {
    ...
    if (response.ok) subscriberCount = Math.max(parseSubscriberCount(await response.text()) ?? 0, subscriberCount);
...
export async function GET() {
  ...
  const cacheIsValid = row?.updated_at.slice(0, 10) === today && parseStoredSubscribers(previous?.topSubscribers) >= topChannel.fallback;
