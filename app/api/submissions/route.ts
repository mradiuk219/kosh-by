...
import { ensureChannelMetricsTable } from '@/lib/channel-metrics';

const OWNER_EMAIL = 'radziuk219@gmail.com';
const DUPLICATE_MESSAGE =
  'Вы спрабуеце прапанаваць канал які ўжо існуе ў каталогу. Калі ласка праверце спасылку.';
...
async function backfillCanonicalKeys(
  ...
  const rows = await db
    .prepare(
      "SELECT * FROM submissions WHERE status IN ('pending', 'approved') AND canonical_key IS NULL ORDER BY CASE status WHEN 'approved' THEN 0 ELSE 1 END, reviewed_at ASC, created_at ASC",
    )
    .all<Submission>();
  for (const item of rows.results ?? []) {
    ...
    try {
      await db
        .prepare('UPDATE submissions SET canonical_key = ? WHERE id = ?')
        .bind(key, item.id)
        .run();
    } catch {
      await db
...
export async function POST(request: Request) {
  ...
  try {
    await db
      .prepare(
        'INSERT INTO submissions (id, url, reason, status, created_at, canonical_key) VALUES (?, ?, ?, ?, ?, ?)',
      )
      .bind(id, url, reason, 'pending', createdAt, canonicalKey)
      .run();
  } catch {
    return Response.json({ error: DUPLICATE_MESSAGE }, { status: 409 });
  }
...
export async function GET(request: Request) {
  ...
  if (view === 'approved') {
    ...
    const result = await db
      .prepare(`SELECT s.id, s.url, s.reason, s.status, s.created_at, s.reviewed_at,
      ...
      .all<
        Omit<Submission, 'submitter_email'> & {
  ...
  await backfillCanonicalKeys(db);
  const result = await db
    .prepare(`SELECT id, url, reason, status, created_at, reviewed_at, title, description,
      category, platform, avatar_url, enrichment_status, canonical_key
      FROM submissions ORDER BY CASE status WHEN 'pending' THEN 0 ELSE 1 END, created_at DESC`)
    .all<Submission>();
  return Response.json({ submissions: result.results ?? [] });
}
...
export async function PATCH(request: Request) {
  ...
  if (!id || !['pending', 'approved', 'rejected'].includes(status))
    return Response.json({ error: 'Няправільныя даныя' }, { status: 400 });
  ...
  if (!submission)
    return Response.json({ error: 'Заяўка не знойдзена' }, { status: 404 });
  let metadata = null;
  if (status === 'approved') {
    try {
      metadata = await enrichChannel(submission.url, submission.reason);
    } catch {
      metadata = null;
    }
  }
  if (status === 'approved' && metadata) {
    try {
      await db
        .prepare(
          "UPDATE submissions SET status = ?, reviewed_at = ?, title = ?, description = ?, category = ?, platform = ?, avatar_url = ?, enrichment_status = 'complete', canonical_key = ? WHERE id = ?",
        )
        .bind(
          status,
    ...
    if (metadata.subscriberCount !== null) {
      const canonicalKey = channelIdentity(submission.url);
      if (canonicalKey) {
  ...
  try {
    await db
      .prepare(
        'UPDATE submissions SET status = ?, reviewed_at = ?, canonical_key = ? WHERE id = ?',
      )
      .bind(
        status,
  ...
  if (status === 'approved')
    await db
      .prepare(
        "UPDATE submissions SET enrichment_status = 'failed' WHERE id = ?",
      )
      .bind(id)
      .run();
  if (status === 'approved')
